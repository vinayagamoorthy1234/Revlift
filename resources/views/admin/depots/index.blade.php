...
@section('content')
	...
	<div class="content">
		<div class="container">
			...
			<div class="row">
				<div class="col-sm-12">
					<h4 class="pull-left page-title">All Depots <a href="{{route('admin.depots.create')}}" class="btn btn-info" style="margin-left:10px;"><i class="fa fa-plus"></i> Add</a></h4>
							...
							<table id="datatable" class="table table-striped table-bordered table-hover">
								...
								<tbody>
									...
									@foreach($depots as $depot)
										<tr class="clickable-row" data-href="{{route('admin.depots.show', ['id'=>$depot->id])}}">
											<td class="text-center actions">
												<a href="{{route('admin.depots.show', ['id'=>$depot->id])}}" title="View" class="text-primary"><i class="fa fa-eye"></i></a>
												<a href="javascript:;" title="Delete" class="sa-warning text-danger" data-depot-id="{{$depot->id}}"><i class="fa fa-trash"></i></a>
											</td>
											<td>{{$depot->code}}</td>
											<td>{{$depot->name}}</td>
											<td class="latitude">{{$depot->latitude}}</td>
											<td class="longitude">{{$depot->longitude}}</td>
											<td>{{date('m/d/Y', strtotime($depot->created_at))}}</td>
											<td>{{date('m/d/Y', strtotime($depot->updated_at))}}</td>
										</tr>
									@endforeach
								</tbody>
							</table>
